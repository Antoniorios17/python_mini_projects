...
def make_csv():
    ...
    for i in range(0,total_synthetic_tests):
        rows.append({'Test Name':response['tests'][i]['name'], 
            ...
            'Status':response['tests'][i]['status'],
            'numOfLocations':len(response['tests'][i]['locations']),
            'Locations':response['tests'][i]['locations'],
            'Creator':response['tests'][i]['creator'],
            'Tags':response['tests'][i]['tags'],
            'Details':response['tests'][i]['config']
            })
        
    with open("Test_File.csv",'w', encoding = 'UTF8', newline = '') as file:
